...
void Model_3DS::Load(char* name)
{
	...
	// strip "'s
	if (strstr(tempName, "\""))
		tempName = strtok_s(tempName, "\"", &context);

	// Find the path
	if (strstr(tempName, "/") || strstr(tempName, "\\"))
	{
		...
		if (strstr(tempName, "/"))
			temp = strrchr(tempName, '/');
		else
			temp = strrchr(tempName, '\\');

		// Allocate space for the path
		path = new char[strlen(tempName) - strlen(temp) + 1];

		// Get a pointer to the end of the path and name
		char* src = tempName + strlen(name) - 1;
		...
		memcpy(path, tempName, src - tempName);
		path[src - tempName] = 0;
	}

	// Load the file
	bin3ds = NULL;
	errno_t err = fopen_s(&bin3ds, tempName, "rb");
	if (bin3ds) 
	{
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
	...
	char fullname[80];
	sprintf_s(fullname, "%s%s", path, n.c_str());
